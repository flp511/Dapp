...
async function main() {
  ...
  const simpleStorage = (await ethers.deployContract(
    "SimpleStorage",
  )) as unknown as SimpleStorage; // 与test3中哪些代码对等???
  let simpleStorageAddress = await simpleStorage.getAddress();
  ...
  if (network.config.chainId === 11155111 && process.env.ETHERSCAN_API_KEY) {
    ...
    await (
      simpleStorage.deploymentTransaction() as ContractTransactionResponse
    ).wait(1);
    ...
    console.log(
      "🚀 ~ file: deploy-simple-storage.js:16 ~ main ~ currentValue:",
    ...
    console.log(
      "🚀 ~ file: deploy-simple-storage.js:20 ~ main ~ updateCurrentValue:",
